...
function editRating() {
    if (finalCheck()) {
        $.ajax({
            url: window.location.origin + "/Rating/Update",
            ...
            success: function (result) {
                ...
                if (result.status == 200) {
                    $.get(
                        window.location.origin + "/Request/Detail?Id=" + ratingData.idRequest,
                        function (data) {
                            $(`.card-${ratingData.idRequest}`).replaceWith(data);
                            showNotification("Thông báo", "Đánh giá đã cập nhật!", 1);
            ...
            error: function (error) {
                console.log(error);
                finalCheck();
                showNotification("Tác vụ thất bại", "Đánh giá không thể cập nhật!", 0);
            }
        });
    }
}
